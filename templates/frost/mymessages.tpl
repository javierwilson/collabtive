...
<h1>{#mymessages#}</h1>
	...
	{section name= project loop=$myprojects}
			...
			<div class="headline">
				...
				<h2>
					<a href="managemessage.php?action=showproject&amp;id={$myprojects[project].ID}" title="{$myprojects[project].name} / {#mymessages#}"><img src="./templates/frost/images/symbols/projects.png" alt="" />{$myprojects[project].name}</a>
				</h2>
			</div>
			...
			<div class="nosmooth" id="sm_{$myprojects[project].ID}">
				...
				<table id="acc_{$myprojects[project].ID}" cellpadding="0" cellspacing="0" border="0">
					...
					<tbody class="color-a" id="task_{$myprojects[project].messages[message].ID}">
					{else}
					<tbody class="color-b" id="task_{$myprojects[project].messages[message].ID}">
					{/if}
						<tr>
							<td>{if $userpermissions.messages.add}<a class="butn_reply" href="managemessage.php?action=replyform&amp;mid={$myprojects[project].messages[message].ID}&amp;id={$myprojects[project].ID}" title="{#answer#}"></a>{/if}</td>
							<td>
								<div class="toggle-in">
								<span class="acc-toggle" onclick="javascript:accord_{$myprojects[project].ID}.activate($$('#acc_{$myprojects[project].ID} .accordion_toggle')[{$smarty.section.message.index}]);toggleAccordeon('acc_{$myprojects[project].ID}',this);"></span>
									<a href="managemessage.php?action=showmessage&amp;mid={$myprojects[project].messages[message].ID}&amp;id={$myprojects[project].ID}" title="{$myprojects[project].messages[message].title}">
										{$myprojects[project].messages[message].title|truncate:35:"...":true}
									</a>
								</div>
							</td>
							<td>
							...
							<td><a href="manageuser.php?action=profile&amp;id={$myprojects[project].messages[message].user}">{$myprojects[project].messages[message].username|truncate:20:"...":true}</a></td>
							...
							<td class="tools">
								...
								<a class="tool_del" href="javascript:confirmit('{#confirmdel#}','managemessage.php?action=del&amp;mid={$myprojects[project].messages[message].ID}&amp;id={$myprojects[project].ID}&redir=managemessage.php?action=mymsgs');"  title="{#delete#}"></a>
								{/if}
							</td>
						</tr>
								...
								<div class="accordion_content">
									<div class="acc-in">
										...
										{else}
												...
												<div class="avatar"><img src = "thumb.php?width=80&amp;height=80&amp;pic=templates/frost/images/no-avatar-female.jpg" alt="" /></div>
												{else}
												<div class="avatar"><img src = "thumb.php?width=80&amp;height=80&amp;pic=templates/frost/images/no-avatar-male.jpg" alt="" /></div>
												{/if}
										{/if}
											...
											{if $myprojects[project].messages[message].files[0][0] > 0}
													...
													<div class="inwrapper">
														...
														{section name = file loop=$myprojects[project].messages[message].files}
															<li>
																<div class="itemwrapper" id="iw_{$myprojects[project].messages[message].files[file].ID}">
																			...
																			<tr>
																				...
																				<td class="thumb">
																				 	<a href = "{$myprojects[project].messages[message].files[file].datei}"{if $myprojects[project].messages[message].files[file].imgfile == 1} rel="lytebox[img{$myprojects[project].messages[message].ID}]" {elseif $myprojects[project].messages[message].files[file].imgfile == 2} rel = "lyteframe[text{$myprojects[project].messages[message].ID}]"{/if} title="{$myprojects[project].messages[message].files[file].name}">
																					 		...
																					 		{else}
																					 		<img src = "templates/frost/images/files/{$myprojects[project].messages[message].files[file].type}.png" alt="" />
																					 		{/if}
																					</a>
																				</td>
																				<td class="rightmen" valign="top">
																					<div class="inmenue">
																						<a class="del" href="managefile.php?action=delete&amp;id={$myprojects[project].ID}&amp;file={$myprojects[project].messages[message].files[file].ID}" title="{#delete#}" onclick="fadeToggle('iw_{$myprojects[project].messages[message].files[file].ID}');"></a>
																						<!-- <a class="edit" href="#" title="{#editfile#}"></a> -->
																					</div>
																				</td>
																			</tr>
																			<tr>
																				<td colspan="3"><span class="name"><a href = "{$myprojects[project].messages[message].files[file].datei}"{if $myprojects[project].messages[message].files[file].imgfile == 1} rel="lytebox[img{$myprojects[project].messages[message].ID}]" {elseif $myprojects[project].messages[message].files[file].imgfile == 2} rel = "lyteframe[text{$myprojects[project].messages[message].ID}]"{/if} title="{$myprojects[project].messages[message].files[file].name}">{$myprojects[project].messages[message].files[file].name|truncate:15:"...":true}</a></span>	</td>
																			<tr/>
																		</table>
