...
<h1>{#administration#}<span>/ {#projectadministration#}</span></h1>
	...
	<div class="headline">
				...
				<h2>
					<img src="./templates/frost/images/symbols/projects.png" alt="" />{#openprojects#}
				</h2>
			</div>
				...
				<table cellpadding="0" cellspacing="0" border="0">
					...
					{/if}
						<tr {if $opros[opro].daysleft < 0} class="marker-late"{elseif $opros[opro].daysleft == 0} class="marker-today"{/if}>
							<td>{if $userpermissions.projects.del}<a class="butn_check" href="javascript:closeElement('proj_{$opros[opro].ID}','manageproject.php?action=close&amp;id={$opros[opro].ID}');" title="{#close#}"></a>{/if}</td>
							<td>
								<div class="toggle-in">
							...
							<td class="b">
								...
								<span class="acc-toggle" onclick="javascript:accord_oldprojects.activate($$('#acc-oldprojects .accordion_toggle')[{$smarty.section.clopro.index}]);toggleAccordeon('acc-oldprojects',this);"></span>
									<a href="manageproject.php?action=showproject&amp;id={$clopros[clopro].ID}" title="{$clopros[clopro].name}">
										{if $clopros[clopro].name != ""}
										{$clopros[clopro].name|truncate:30:"...":true}
										{else}
										{$clopros[clopro].desc|truncate:30:"...":true}
										{/if}
									</a>
								</div>
							</td>
