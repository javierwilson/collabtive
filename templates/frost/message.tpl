...
<a href="manageproject.php?action=showproject&amp;id={$project.ID}"><img src="./templates/frost/images/symbols/projects.png" alt="" />{$projectname|truncate:40:"...":true}</a>
<a href="managemessage.php?action=showproject&amp;id={$project.ID}"><img src="./templates/frost/images/symbols/msgs.png" alt="" />{#messages#}</a>
</div>

<h1 class="second"><img src="./templates/frost/images/symbols/msgs.png" alt="" />{$message.title|truncate:40:"...":true}</h1>
	...
	<div id="message" class="descript">
		...
		<div class="message">
			<div class="message-in">
				<h2>{$message.endstring}&nbsp;/&nbsp;
				{$message.username|truncate:20:"...":true}</h2>
				{$message.text}
			</div>
												...
												<tr>
													...
													<td class="thumb">
													 	<a href = "{$ordner[file].datei}"{if $ordner[file].imgfile == 1} rel="lytebox[img{$ordner[file].ID}]" {elseif $ordner[file].imgfile == 2} rel = "lyteframe[text{$ordner[file].ID}]"{/if} title="{$ordner[file].name}">
													 		...
													 		{else}
													 		<img src = "templates/frost/images/files/{$ordner[file].type}.png" alt="{$ordner[file].name}" />
													 		{/if}
													 	</a>
													</td>
	...
	{if $replies > 0}
				...
				<h2>
					<img src="./templates/frost/images/symbols/msgs.png" alt="" />{#replies#}</a>
				</h2>
			</div>
				...
				<table id="acc_replies" cellpadding="0" cellspacing="0" border="0">
					...
					{/if}
						<tr>
							...
							<td><a href="manageuser.php?action=profile&amp;id={$replies[reply].user}">{$replies[reply].username|truncate:20:"...":true}</a></td>
							<td class="tools">
								{if $userpermissions.messages.edit}
								<a class="tool_edit" href="managemessage.php?action=editform&amp;mid={$replies[reply].ID}&amp;id={$message.project}" title="{#edit#}"></a>
								{/if}
								{if $userpermissions.messages.del}
								<a class="tool_del" href="javascript:confirmit('{#confirmdel#}','managemessage.php?action=del&amp;mid={$replies[reply].ID}&amp;id={$message.project}');"  title="{#delete#}"></a>
								{/if}
							</td>
						</tr>
											...
											<strong>{#files#}:</strong>
													<div class="inwrapper">
														...
														{section name = file loop=$replies[reply].files}
															<li>
																<div class="itemwrapper" id="iw_{$replies[reply].files[file].ID}">
																			...
																			<tr>
																				...
																				<td class="thumb">
																				 	<a href = "{$replies[reply].files[file].datei}"{if $replies[reply].files[file].imgfile == 1} rel="lytebox[img{$replies[reply].files[file].ID}]" {elseif $replies[reply].files[file].imgfile == 2} rel = "lyteframe[text{$replies[reply].files[file].ID}]"{/if} title="{$replies[reply].files[file].name}">
																					 		...
																					 		{else}
																					 		<img src = "templates/frost/images/files/{$replies[reply].files[file].type}.png" alt="" />
																					 		{/if}
																					</a>
																				</td>
																				<td class="rightmen" valign="top">
																					<div class="inmenue">
																						<a class="del" href="managefile.php?action=delete&amp;id={$myprojects[project].ID}&amp;file={$replies[reply].files[file].ID}" title="{#delete#}" onclick="fadeToggle('iw_{$replies[reply].files[file].ID}');"></a>
																						<!-- <a class="edit" href="#" title="{#editfile#}"></a> -->
																					</div>
																				</td>
																			</tr>
																			<tr>
																				<td colspan="3"><span class="name"><a href = "{$replies[reply].files[file].datei}"{if $replies[reply].files[file].imgfile == 1} rel="lytebox[img{$replies[reply].files[file].ID}]" {elseif $replies[reply].files[file].imgfile == 2} rel = "lyteframe[text{$replies[reply].files[file].ID}]"{/if} title="{$replies[reply].files[file].name}">{$replies[reply].files[file].name|truncate:15:"...":true}</a></span></td>
																			<tr/>
																		</table>
