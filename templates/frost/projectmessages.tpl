...
<div class="msgs">
	...
	<span id = "deleted" style = "display:none;" class="info_in_red"><img src="templates/frost/img/symbols/msg.png" alt=""/>{#messagewasdeleted#}</span>
	</div>

	<div class="infowin_left" style = "display:none;" id = "systemmsg">
		{if $mode == "added"}
		<span class="info_in_green"><img src="templates/frost/images/symbols/msgs.png" alt=""/>{#messagewasadded#}</span>
		{elseif $mode == "edited"}
		<span class="info_in_yellow"><img src="templates/frost/images/symbols/msgs.png" alt=""/>{#messagewasedited#}</span>
		{elseif $mode == "replied"}
		<span class="info_in_green"><img src="templates/frost/images/symbols/msgs.png" alt=""/>{#replywasadded#}</span>
		{/if}
	</div>
	{literal}
...
<h1>{$projectname}<span>/ {#messages#}</span></h1>
			...
			<div class="headline">
				...
				{if $userpermissions.messages.add}<a class="add" href="javascript:blindtoggle('addmsg{$myprojects[project].ID}');" id="add" onclick="toggleClass(this,'add-active','add');toggleClass('add_butn','butn_link_active','butn_link');toggleClass('sm_msgs','smooth','nosmooth');"><span>{#addmessage#}</span></a>{/if}


                    </div>

				<h2>
					<img src="./templates/frost/images/symbols/msgs.png" alt="" />{#messages#}
				</h2>
			</div>
			...
			<div class="nosmooth" id="sm_msgs">
				...
				<table id="acc_msgs" cellpadding="0" cellspacing="0" border="0">
					...
					{/if}
						<tr>
							<td>{if $userpermissions.messages.close}<a class="butn_reply" href="managemessage.php?action=replyform&amp;mid={$messages[message].ID}&amp;id={$project.ID}" title="{#answer#}"></a>{/if}</td>
							<td>
								<div class="toggle-in">
								<span class="acc-toggle" onclick="javascript:accord_messages.activate($$('#block_msgs .accordion_toggle')[{$smarty.section.message.index}]);toggleAccordeon('accord_messages',this);"></span>
									<a href="managemessage.php?action=showmessage&amp;mid={$messages[message].ID}&amp;id={$project.ID}" title="{$messages[message].title}">
										{$messages[message].title|truncate:35:"...":true}
									</a>
								</div>
							</td>
							<td>
							...
							<td class="tools">
								...
								<a class="tool_edit" href="managemessage.php?action=editform&amp;mid={$messages[message].ID}&amp;id={$project.ID}" title="{#edit#}"></a>{/if}
								{if $userpermissions.messages.del}
								<a class="tool_del" href="javascript:confirmit('{#confirmdel#}','managemessage.php?action=del&amp;mid={$messages[message].ID}&amp;id={$project.ID}&redir=managemessage.php?action=mymsgs');"  title="{#delete#}"></a>
								{/if}
							</td>
						</tr>
								...
								<div class="accordion_content">
									<div class="acc-in">
										...
										{else}
												...
												<div class="avatar"><img src = "thumb.php?pic=templates/frost/images/no-avatar-female.jpg&amp;width=80;" alt="" /></div>
												{else}
												<div class="avatar"><img src = "thumb.php?pic=templates/frost/images/no-avatar-male.jpg&amp;width=80;" alt="" /></div>
												{/if}
										{/if}
																				...
																				<td class="thumb">
																				 	<a href = "{$messages[message].files[file].datei}"{if $messages[message].files[file].imgfile == 1} rel="lytebox[img{$messages[message].ID}]" {elseif $messages[message].files[file].imgfile == 2} rel = "lyteframe[text{$messages[message].ID}]"{/if} title="{$messages[message].files[file].name}">
																						{if $messages[message].files[file].imgfile == 1}
																				 		...
																				 		{else}
																						<img src = "templates/frost/images/files/{$messages[message].files[file].type}.png" alt="" />
																				 		{/if}
																					</a>
																				</td>
