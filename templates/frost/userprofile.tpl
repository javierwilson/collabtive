...
<tr>
	...
	{else}
		{if $user.gender == "f"}
		<div class="avatar-profile"><img src = "thumb.php?pic=templates/frost/images/no-avatar-female.jpg&amp;width=122;" alt="" /></div>
		{else}
		<div class="avatar-profile"><img src = "thumb.php?pic=templates/frost/images/no-avatar-male.jpg&amp;width=122;" alt="" /></div>
		{/if}
	{/if}
...
<div class="projects">
			<div class="headline">
				...
				<h2>
					<a href="myprojects.php" title="{#myprojects#}"><img src="./templates/frost/images/symbols/projects.png" alt="" />{#projects#}</a>
				</h2>
			</div>
				...
				<table cellpadding="0" cellspacing="0" border="0">
					...
					{/if}
						<tr {if $opros[opro].daysleft < 0} class="marker-late"{elseif $opros[opro].daysleft == 0} class="marker-today"{/if}>
							<td>{if $adminstate > 4}<a class="butn_check" href="javascript:closeElement('proj_{$opros[opro].ID}','manageproject.php?action=close&amp;id={$opros[opro].ID}');" title="{#close#}"></a>{/if}</td>
							<td>
								<div class="toggle-in">
								<span class="acc-toggle" onclick="javascript:accord_projects.activate($$('#projecthead .accordion_toggle')[{$smarty.section.opro.index}]);toggleAccordeon('projecthead',this);"></span>
									<a href="manageproject.php?action=showproject&amp;id={$opros[opro].ID}" title="{$opros[opro].name}">
										...
										{else}
										{$opros[opro].desc|truncate:30:"...":true}
										{/if}
									</a>
								</div>
							</td>
												...
												<li>
													<div class="itemwrapper" id="iw_{$opros[opro].ID}_{$opros[opro].members[member].ID}">
																		...
																		<table cellpadding="0" cellspacing="0" border="0">
																			<tr>
																				...
																				<td class="thumb">
																				 	<a href="manageuser.php?action=profile&amp;id={$opros[opro].members[member].ID}" title="{$opros[opro].members[member].name}">
																						...
																						<img src = "./templates/frost/images/symbols/user-icon-female.png" alt="" />
																						{else}
																						<img src = "./templates/frost/images/symbols/user-icon-male.png" alt="" />
																						{/if}
																					</a>
																				</td>
																			...
																			<tr>
																				<td colspan="3"><span class="name"><a href="manageuser.php?action=profile&amp;id={$opros[opro].members[member].ID}" title="{$opros[opro].members[member].name}">{$opros[opro].members[member].name|truncate:15:"...":true}</a></span></td>
																			<tr/>
																		</table>
...
<div class="timetrack">
			<div class="headline">
				...
				<h2>
					<img src="./templates/frost/images/symbols/timetracker.png" alt="" />{#report#}
				</h2>
			</div>
							...
							<td>
								...
								<span class="acc-toggle" onclick="javascript:accord_tracker.activate($$('#acc-tracker .accordion_toggle')[{$smarty.section.track.index}]);toggleAccordeon('acc-tracker',this);"></span>
									<a href = "managetimetracker.php?action=showproject&amp;id={$tracker[track].project}" title="{$tracker[track].pname}">
										{$tracker[track].pname|truncate:16:"...":true}
									</a>
								</div>
							</td>
