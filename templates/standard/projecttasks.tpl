...
{section name=list loop=$lists}
			...
			<div class="headline">
				...
				<h2>
					<a href="managetasklist.php?action=showtasklist&amp;id={$project.ID}&amp;tlid={$lists[list].ID}" title="{#tasklist#} {$lists[list].name}"><img src="./templates/standard/images/symbols/tasklist.png" alt="" />{$lists[list].name}</a>
				</h2>
				...
				<table id="acc_{$lists[list].ID}" cellpadding="0" cellspacing="0" border="0">
					...
					{/if}
						<tr {if $lists[list].tasks[task].daysleft < 0} class="marker-late"{elseif $lists[list].tasks[task].daysleft == 0} class="marker-today"{/if}>
							...
							<td>{section name=theusers loop=$lists[list].tasks[task].users}<a href="manageuser.php?action=profile&amp;id={$lists[list].tasks[task].users[theusers].ID}">{$lists[list].tasks[task].users[theusers].name|truncate:30:"...":true}</a> {/section}</td>
					...
					<table cellpadding="0" cellspacing="0" border="0" id = "done_{$lists[list].ID}">
						...
						{/if}
							...
							<tr>
								...
								<td class="days">{$lists[list].oldtasks[oldtask].daysleft}</td>
								<td class="tools">
									{if $userpermissions.tasks.edit}
