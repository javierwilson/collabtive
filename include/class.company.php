...
class company
{
    ...
    */
    function add($name, $email, $phone, $address1, $address2, $state, $country, $logo)
    {
        ...
        $logo = pg_escape_string($logo);

        $ins1 = pg_query("INSERT INTO company (ID, name, email, phone, address1, address2, state, country, logo) VALUES ('', '$name', '$email', '$phone', '$address1', '$address2', '$state', '$country', '$logo')");
    ...
    */
    function edit($id, $name, $email, $phone, $address1, $address2, $state, $country, $logo)
    {
        ...
        $upd = pg_query("UPDATE company SET name='$name', email='$email', phone='$phone', address1='$address1', address2='$address2', state='$state', country='$country', logo='$logo' WHERE ID = $id");
        if ($upd)
        {
    ...
    */
    ...
    {
        $id = (int) $id;
        $del = pg_query("DELETE FROM company WHERE ID = $id");
        if ($del)
        {
    ...
    */
	function assign($company,$user)
	{
		...
		$ins = pg_query("UPDATE user SET company=$company WHERE ID = $id");
		if($ins)
		{
			return true;
		}
    ...
    */
	function deassign($company,$user)
	{
		...
		$ins = pg_query("UPDATE user SET company=0 WHERE ID = $id");
		if($ins)
		{
			return true;
		}
    ...
    */
    function getProfile($id)
    {
        ...
        $sel = pg_query("SELECT * FROM company WHERE ID = $id");
	...
	{
		$sel = pg_query("SELECT * FROM company");
		$companies = array();
		
		while($company = pg_fetch_array($sel))
		{
    ...
    */
    function getCompanyMembers($id)
    {
        ...
        $sel = pg_query("SELECT user,company FROM company_assigned WHERE company = $id");
        $staff = array();
        $userobj = (object) new user();
        $company = $this->getProfile($member[1]);
        while($member = pg_fetch_row($sel))
        {
